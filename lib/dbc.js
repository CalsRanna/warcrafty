...
const { camelCase, kebabCase, snakeCase } = require("../lib/util");
          ...
          for (let key of keys) {
            switch (entity[key]) {
              ...
              case t.float32le:
                record[key] =
                  Math.round(
                    (buffer.readFloatLE(offset) + Number.EPSILON) * 10000
                  ) / 10000;
                offset = offset + 4;
                break;
              case t.string:
    ...
    for (let record of records) {
      for (let key of keys) {
        switch (entity[key]) {
          ...
          case t.float32le:
            let value =
              (Math.round(record[key] + Number.EPSILON) * 1000) / 1000;
...
function toSql(fpath, database="foxy"){
  ...
  return `CREATE TABLE IF NOT EXISTS \`${database}\`.\`dbc_${snakeCase(name)}\` (${sql} PRIMARY KEY(\`${keys[0]}\`)) ENGINE=MyISAM DEFAULT CHARSET=utf8mb3;`
}
